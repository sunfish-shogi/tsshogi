...
export class Move {
  ...
  get usi(): string {
    ...
    if (this.isDrop) {
      ...
      ret += this.fromSquare.usi;
    }
    ret += this.toSquare.usi;
    if (this.promote) {
      ret += "+";
    }
    return ret;
...
export function parseUSIMove(usiMove: string): {
  ...
  const toSquare = Square.newByUSI(usiMove.substring(2));
  if (!toSquare) {
    return null;
  }
  const promote = usiMove.length >= 5 && usiMove[4] === "+";
  return { from, to: toSquare.index, promote };
}
